...
const UserCard = async ({user}: Props) => {
    const interaactedTags = await getTopInteractedTags({userId: user._id})
    return (
        <Link
        ...
        >
            <article
                ...
                <div className="mt-5">
                    {interaactedTags && interaactedTags.length > 0 ?
                        (
                            <div className="flex items-center gap-2">
                                {interaactedTags.map((tag) => (
                                    <RenderTag id={tag._id} key={tag._id} title={tag.name} />
